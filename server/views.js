...
function renderFocusedLoginPage(errorMsg = null) {
  ...
  <div class="view" id="discord-code"><button class="back" data-view="login-main">← Tüm giriş yöntemleri</button><h3>Discord’dan kod al</h3><p class="sub">Kullanıcı adını yaz; Phibi/Sentara kısa süreli dört haneli kodu Discord DM’ine gönderir.</p><div class="option-note">Kodu hiç kimseyle paylaşma. <a href="/blog/discord-ile-giris-nasil-calisir">Bu yöntem nasıl çalışır?</a></div><div id="dm-start"><div class="field"><label>Discord kullanıcı adı veya ID</label><input id="dm-username" autocomplete="username" placeholder="ör. ekoyildiz"></div><button class="primary" id="dm-request">Kodu gönder</button></div><div id="dm-verify" style="display:none"><div class="field"><label>DM’deki 4 haneli kod</label><input id="dm-code" inputmode="numeric" maxlength="4" placeholder="0000"></div><button class="primary" id="dm-confirm">Doğrula ve giriş yap</button></div></div>
  ...
  let resolvedDiscord='';document.getElementById('dm-request').addEventListener('click',async()=>{const username=document.getElementById('dm-username').value.trim();if(!username)return showError('Discord kullanıcı adı veya ID gerekli.');const btn=document.getElementById('dm-request');btn.disabled=true;btn.textContent='Kod gönderiliyor…';try{const r=await fetch('/auth/send-discord-dm-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username})});const d=await r.json();if(d.success){resolvedDiscord=d.targetId||d.discordId||username;document.getElementById('dm-start').style.display='none';document.getElementById('dm-verify').style.display='block'}else showError(d.error||'Kod gönderilemedi.')}catch(e){showError('Bağlantı kurulamadı.')}btn.disabled=false;btn.textContent='Kodu gönder'});
  document.getElementById('dm-confirm').addEventListener('click',async()=>{const code=document.getElementById('dm-code').value.trim();if(code.length!==4)return showError('DM’deki 4 haneli kodu yaz.');try{const r=await fetch('/api/auth/verify-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({discordId:resolvedDiscord,code,rememberMe:true})});const d=await r.json();if(d.success)location.href=d.redirectUrl||'/dashboard';else showError(d.error||'Kod doğrulanamadı.')}catch(e){showError('Bağlantı kurulamadı.') }});
