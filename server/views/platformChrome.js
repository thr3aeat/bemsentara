...
function platformChromeStyles(theme = 'dark') {
  ...
  return `<style>
    ...
    .platform-dropdown{position:relative;display:inline-flex;align-items:center;flex:0 0 auto;z-index:3}
    ...
    .platform-dropdown-menu{position:absolute;top:calc(100% + 8px);left:50%;transform:translateX(-50%) translateY(4px);min-width:190px;max-width:calc(100vw - 32px);padding:7px;background:var(--liquid-lens);border:1px solid var(--liquid-border);border-radius:16px;box-shadow:var(--liquid-shadow);backdrop-filter:blur(30px) saturate(220%);-webkit-backdrop-filter:blur(30px) saturate(220%);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .2s ease,transform .2s cubic-bezier(0.34,1.56,0.64,1),visibility .2s;z-index:1000;display:flex;flex-direction:column;gap:3px}
