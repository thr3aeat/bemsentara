...
<body>
  <div class="home-wrap">
    <header class="home-nav"><a class="home-brand" href="/">eko<i>yıldız</i></a><nav class="home-links"><a href="/video-blog">Video Blog</a><a href="/yardim">Yardım merkezi</a><a href="/cekilisler">Çekilişler</a>${accountAction}</nav></header>
    <main>
  ...
  </div><script>const starButton=document.getElementById('star-button');let stars=0;starButton?.addEventListener('click',e=>{stars++;for(let i=0;i<7;i++){const s=document.createElement('span');s.className='floating-star';s.textContent='✦';s.style.left=(e.clientX+(Math.random()*70-35))+'px';s.style.top=(e.clientY+(Math.random()*30-15))+'px';s.style.setProperty('--x',(Math.random()*140-70)+'px');document.body.appendChild(s);setTimeout(()=>s.remove(),950)}if(stars===5){starButton.title='EkoYıldız modu açık ✦';starButton.textContent='★';}});document.querySelector('.home-brand')?.addEventListener('dblclick',()=>{document.body.style.background='#fff8fa';setTimeout(()=>document.body.style.background='',900)});</script>
</body></html>`;
}
