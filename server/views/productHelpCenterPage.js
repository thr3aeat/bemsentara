...
const helpCategories = [
  { slug: 'hesap-ve-giris', icon: '🔐', title: 'Hesap ve giriş', description: 'Giriş yöntemleri, Discord hesabını bağlama, doğrulama ve hesap ayarları.', links: [['Giriş seçenekleri', '/login'], ['Hesap ayarları', '/settings']] },
  { slug: 'ekoyildiz-bot', icon: '🤖', title: 'EkoYıldız Bot', description: 'Bot komutları, izinler, DM bildirimleri ve yaygın sorunların çözümü.', links: [['Bot rehberi', '/blog/phibi-bot-nasil-kullanilir'], ['Bot desteği', '/help/bot']] },
  { slug: 'panel-ve-ozellikler', icon: '🧭', title: 'Panel ve özellikler', description: 'Dashboard, profil, bildirimler, çekilişler ve topluluk araçlarını kullanma.', links: [['Paneli aç', '/dashboard'], ['Çekilişler', '/cekilisler']] },
  { slug: 'ticket-ve-destek', icon: '🎫', title: 'Ticket ve destek', description: 'Yeni destek talebi gönder, biletlerini izle ve teslim sorunlarını çöz.', links: [['Yeni ticket aç', '/tickets/new'], ['Sorun giderici', '/yardim/ticket-sorun-giderici']] },
  { slug: 'moderasyon-ve-itiraz', icon: '⚖️', title: 'Moderasyon ve itiraz', description: 'Bir işlemi anlamak, kanıt hazırlamak ve uygun itiraz kanalına ulaşmak.', links: [['İtiraz merkezi', '/appeals'], ['Ceza rehberi', '/yardim/ceza-ve-itiraz']] },
  { slug: 'guvenlik-ve-safety', icon: '🛡️', title: 'Güvenlik ve Safety', description: 'Phishing, scam, hesap güvenliği, raporlama ve topluluk güvenliği rehberleri.', links: [['Safety Center', '/safety'], ['Raporlama rehberi', '/yardim/kullanici-raporlama']] },
];
...
function renderProductHelpCenterPage(user = null) {
  const cards = helpCategories.map((category) => `<article class="help-card" id="${category.slug}"><span class="help-icon" aria-hidden="true">${category.icon}</span><h2>${esc(category.title)}</h2><p>${esc(category.description)}</p><div class="help-links">${category.links.map(([label, url]) => `<a href="${url}">${esc(label)} <span aria-hidden="true">→</span></a>`).join('')}</div></article>`).join('');
