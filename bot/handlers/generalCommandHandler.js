...
Bu personelin bugünkü görevleri henüz başlatmadığını belirten çok kısa (max 100 karakter), neşeli ve motive edici Türkçe bir karşılama yaz.`;
  ...
  try {
    ...
    if (commandName === "incele") {
      const { handleInceleCommand } = require("../services/userManagementService");
      return await handleInceleCommand(interaction);
    }
...
async function generateInceleData(guild, targetUser, targetMember) {
  const { fetchComprehensiveUserData, buildIncelePayload } = require("../services/userManagementService");
  const data = await fetchComprehensiveUserData(targetUser.id, guild, guild?.client);
  return buildIncelePayload(data, "overview", targetMember);
}
